(function () {
  ...
  const SKILL_EFFECT_ASSETS = {
    '불꽃':'skill-effects/fire.gif', '물':'skill-effects/water.gif', '풀':'skill-effects/grass.gif', '전기':'skill-effects/elec.gif',
    '고스트':'skill-effects/darkness.gif', '격투':'skill-effects/fist.gif', '독':'skill-effects/poison.gif',
    '드래곤':'skill-effects/dragon.gif', '노말':'skill-effects/normal.gif', '비행':'skill-effects/wing.gif',
    '땅':'skill-effects/sand.gif', '얼음':'skill-effects/ice.gif', '바위':'skill-effects/silla.gif',
    '악':'skill-effects/darkpulse.gif', '에스퍼':'skill-effects/esper.gif', '강철':'skill-effects/steel.gif', '벌레':'skill-effects/insect.gif'
  };
  ...
  function queueBossCry(name) {
    const map = { '다크라이':'darkrai_cry.mp3', '포푸니라':'weavile_cry.mp3', '기라티나':'giratina_cry.mp3' };
    const src = map[String(name || '').trim()];
    if (src) queueEffect({ type: 'cry', src });
  }
  ...
  function startBattle(options) {
    ...
    state.waiting = Boolean(options?.trainerIntroSrc);
    state.uiEffects.length = 0;
    state.log.length = 0;
    ...
    if (options?.mode === 'dungeon') queueBossCry(getActive('enemy')?.currentName);
    render();
    if (state.waiting) { window.setTimeout(() => { if (state.active) { state.waiting = false; render(); } }, 2100); }
    return true;
  }
  ...
  function performMove(side, moveIndex) {
    ...
    const effectAsset = SKILL_EFFECT_ASSETS[move?.type];
    if (effectAsset) {
      queueEffect({ type: 'skill', asset: effectAsset, label: `${attacker.currentName} · ${move.nameKo}` });
      queueEffect({ type: 'sound', name: 'hit' });
    }
  ...
  function finishBattle(winnerId, loserId, fled = false) {
    ...
    const applyPostBattleLevelReward = () => {
      ...
      rewarded.forEach((id) => {
        POKEBATTLE.core?.levelUpWholeTeam?.(id, 10);
    ...
    if (!fled && winnerId !== state.playerId) setTimeout(() => POKEBATTLE.ui?.playEffectSound?.('fail.mp3'), 1000);
    state.forceSwitch = false;
    state.forceSwitchSide = null;
    state.optionalSwitchSide = null;
    state.active = false;
